/*
Problem: Determine if all courses/tasks can be finished given prerequisites.

Approach:
1. Build a directed graph where an edge preTask -> task indicates dependency.
2. Use DFS with 3-state visited array:
   - 0: unvisited
   - 1: visiting (currently in DFS stack)
   - 2: visited (DFS complete)
3. During DFS, if a node in state 1 is visited again, a cycle exists.
4. If any cycle is detected, return false; otherwise, return true.

Example:
n = 4, prerequisites = [[1,0],[2,1],[3,2]]
Graph edges: 0->1, 1->2, 2->3
No cycle exists, so all tasks can be finished.
Output: true
*/
