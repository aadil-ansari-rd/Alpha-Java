/*
Tarjan's Algorithm for Strongly Connected Components (SCC) in Java

Problem:
Given a directed graph, find all strongly connected components (SCCs).

Approach:
1. Initialize discovery time (disc) and low-link values (low) for all vertices.
2. Perform DFS:
   - For each unvisited vertex, recursively visit neighbors.
   - Update low[u] based on neighbors' discovery times and low-link values.
   - Use a stack to track vertices in the current DFS path.
3. When disc[u] == low[u], pop stack to form an SCC.
4. Return list of all SCCs.

Key Concepts:
- low[u] represents the earliest visited vertex reachable from u.
- Vertices in the same SCC have interconnected low-link values.
