/*
Problem Statement: Find the cheapest flight from src to dst with at most k stops.

Approach (BFS-based):
1. Build adjacency list of flights.
2. Use a queue storing (city, cost, stops).
3. Initialize dist[] array with MAX_VALUE, dist[src] = 0.
4. BFS traversal:
   - For current city, if stops > k, skip.
   - For each neighbor city, calculate newCost = cost + edge weight.
   - If newCost < dist[neighbor], update dist[neighbor] and enqueue (neighbor, newCost, stops+1).
5. Return dist[dst] or -1 if unreachable.

Example:
n = 4, flights = [[0,1,100],[1,2,100],[2,3,100],[0,2,500]], src=0, dst=3, k=1
Output: Cheapest Price = 200 (via 0->1->2->3 is within k+1 stops)
*/
