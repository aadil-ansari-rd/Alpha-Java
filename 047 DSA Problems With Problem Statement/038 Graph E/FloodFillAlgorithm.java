/*
Flood Fill Algorithm (DFS approach) in Java:

1. `floodFill(image, sr, sc, newColor)`:
   - Entry point. Stores the original color at starting pixel.
   - Calls DFS `fill()` only if the new color differs from original.

2. `fill(image, row, col, originalColor, newColor)`:
   - Base case: check bounds and color match.
   - Changes current pixel to `newColor`.
   - Recursively explores 4 directions: up, down, left, right.
