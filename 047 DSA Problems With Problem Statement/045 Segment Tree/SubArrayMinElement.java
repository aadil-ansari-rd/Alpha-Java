/*
Problem: Range Minimum Query (RMQ) using Segment Tree

Given an array arr[], build a segment tree to efficiently perform the following operations:

1. Query the minimum element in a subarray [l, r].
2. Update a value at a specific index and reflect the change in the segment tree.

Example:
Input: arr = [6, 8, -1, 2, 17, 1, 3, 2, 4]
Operations:
- Query min in range [0, 4] → Output: -1
- Query min in range [2, 5] → Output: -1
- Query min in range [6, 8] → Output: 2
- Update arr[2] = 10
- Query min in range [0, 4] → Output: 2
- Update arr[3] = -5
- Query min in range [0, 4] → Output: -5
- Query min in range [0, 8] → Output: -5
*/
