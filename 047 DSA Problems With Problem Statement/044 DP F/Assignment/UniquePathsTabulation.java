/*
Unique Paths in Grid (Tabulation / Bottom-Up DP)

Problem:
- Grid of size m x n
- Start at (0,0), move only right or down
- Count unique paths to (m-1,n-1)

Approach:
1. Create a DP table dp[m][n], where dp[i][j] = ways to reach cell (i,j)
2. Initialize first row and column to 1 (only one path along edges)
3. Fill remaining cells: dp[i][j] = dp[i-1][j] + dp[i][j-1]
4. Return dp[m-1][n-1] as the answer
