/*
Problem: Target Sum Subset (Tabulation)

Given an array arr[] and a target sum, determine if a subset exists with sum equal to target.

Approach (DP Tabulation):
1. dp[i][j] = true if a subset of first i elements can form sum j.
2. Base case: dp[i][0] = true (sum 0 always possible), dp[0][j] = false for j>0.
3. Recurrence:
   dp[i][j] = dp[i-1][j] || (j >= arr[i-1] ? dp[i-1][j-arr[i-1]] : false)
   - Exclude current element OR include it if feasible.
4. Answer: dp[n][target]
