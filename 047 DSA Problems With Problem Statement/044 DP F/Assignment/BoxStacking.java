/*
Problem: Box Stacking (Maximum Height) using DP

Given n boxes, each with dimensions (length, width, height), find the tallest stack possible such that each box above has strictly smaller length and width than the box below. Boxes can be rotated to use any side as base.

Example:
Input: boxes = [[4,6,7],[1,2,3],[4,5,6],[10,12,32]]
Output: Maximum Stack Height = 60

Approach:
1. Generate all 3 rotations for each box (length >= width for base consistency).
2. Sort boxes by decreasing base area (length*width).
3. Initialize dp[i] = height of box i.
4. For each box i, check all boxes j < i:
   - If box i can sit on box j (both length & width smaller), update dp[i] = max(dp[i], dp[j]+height[i]).
5. Answer = max(dp[i]) for all i.
