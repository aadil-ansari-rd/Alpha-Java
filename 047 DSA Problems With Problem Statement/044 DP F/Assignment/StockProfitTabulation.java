/*
Problem: Buy and Sell Stock II (Tabulation)

Given an array prices[], find the maximum profit by buying and selling as many times as you like (but cannot hold multiple stocks simultaneously).

Approach (DP Tabulation):
1. dp[i][0] = max profit on day i if NOT holding stock.
2. dp[i][1] = max profit on day i if holding stock.
3. Base case:
   dp[0][0] = 0 (not holding stock day 0)
   dp[0][1] = -prices[0] (buy stock on day 0)
4. Recurrence:
   dp[i][0] = max(dp[i-1][0], dp[i-1][1] + prices[i])
   dp[i][1] = max(dp[i-1][1], dp[i-1][0] - prices[i])
5. Answer: dp[n-1][0] (not holding stock at the end).
