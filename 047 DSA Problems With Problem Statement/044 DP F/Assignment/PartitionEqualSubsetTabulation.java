/*
Problem: Partition Equal Subset Sum (Tabulation)
...
2. Set target = sum / 2.
3. Create dp[i][j] = true if a subset of first i numbers can sum to j.
4. Base case: dp[i][0] = true (sum 0 is always possible).
5. Recurrence: 
   dp[i][j] = dp[i-1][j] || dp[i-1][j-nums[i-1]] (include/exclude current number).
