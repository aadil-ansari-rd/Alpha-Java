/*
Unique Paths II (with Obstacles) - Tabulation DP

Problem:
- Grid of size m x n with obstacles (1 = obstacle, 0 = free)
- Start at (0,0), move only right or down
- Count unique paths to (m-1,n-1) avoiding obstacles

Approach:
1. If start cell is blocked, return 0.
2. Create DP table dp[m][n], dp[i][j] = ways to reach cell (i,j)
3. Initialize first row and first column:
   - dp[i][0] = 1 if no obstacle above and current cell free
   - dp[0][j] = 1 if no obstacle left and current cell free
4. Fill rest of table:
