...
Given a binary tree, maximize sum of node values such that no two directly-linked nodes are robbed.

Approach (DFS with DP):
1. For each node, return an array [notRob, rob]:
   - notRob = max money if node is NOT robbed
   - rob = max money if node IS robbed
2. Recursively calculate left and right child DP values.
3. Formulas:
   - notRob = max(left[0], left[1]) + max(right[0], right[1])
   - rob = node.val + left[0] + right[0]
4. Result = max(root[0], root[1])
