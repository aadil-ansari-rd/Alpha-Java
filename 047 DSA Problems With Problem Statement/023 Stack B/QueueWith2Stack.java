/*
Problem Name: Queue Using 2 Stacks
-----------------------------------
Problem Statement:
-----------------
Implement a Queue using two stacks with the following approaches:

1. Lazy Enqueue, Eager Dequeue (Queue1)
   - Enqueue: O(1)
   - Dequeue / Peek: Amortized O(1), Worst O(n)
   - Logic:
     * Push elements into s1.
     * On dequeue/peek, if s2 is empty, transfer all elements from s1 to s2 (reversing order).
     * Pop from s2.

2. Eager Enqueue, Lazy Dequeue (Queue2)
   - Enqueue: O(n)
   - Dequeue / Peek: O(1)
   - Logic:
     * Move all elements from s1 to s2, push new element into s1, then move back.
     * s1 always has elements in queue order, so dequeue/peek is direct.

Key Points:
-----------
- Two stacks can simulate a queue by reversing order using one stack as auxiliary.
- Lazy enqueue is preferred when enqueue operations are frequent.
- Eager enqueue is preferred when dequeue operations are frequent.
- Display logic in lazy enqueue needs careful handling to show elements in correct order.

Example:
--------
Queue1: enqueue 10, 20, 30 → display: [10, 20, 30]
peek() → 10, dequeue() → 10

Queue2: enqueue 100, 200, 300 → display: [100, 200, 300]
peek() → 100, dequeue() → 100
*/
