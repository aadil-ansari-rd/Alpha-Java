/*
Problem Name: Stack Using Two Queues
------------------------------------
Problem Statement:
-----------------
Implement a stack using two queues with two main approaches:

1. Costly Push (Efficient Pop)
   - push(x): O(n)
     • Push new element into empty q2.
     • Move all elements from q1 to q2.
     • Swap q1 and q2.
   - pop()/peek(): O(1) → Directly from front of q1.

2. Costly Pop (Efficient Push)
   - push(x): O(1) → Simply enqueue to q1.
   - pop()/peek(): O(n) worst case (Amortized O(1) for sequence)
...
Key Points:
-----------
- Costly Push is preferred when pop operations are frequent.
- Costly Pop is preferred when push operations are frequent.
- Both approaches maintain LIFO order using two queues.
- Displaying elements in stack order may require reversing queue order.
*/
