/*
Problem: Cycle Detection in Undirected Weighted Graph using DFS

Given a weighted undirected graph, determine whether the graph contains any cycle. A cycle exists if there is a path starting and ending at the same vertex with at least one edge, without retracing any edge.

You are required to:
1. Represent the graph using adjacency lists with edges containing source, destination, and weight.
2. Add edges to the graph.
3. Detect cycles in any component of the graph using DFS.
