/*
Problem Statement: Implement a Custom HashMap

Implement a generic HashMap class in Java that supports basic operations including:
- put(K key, V value): Insert or update key-value pairs
- get(K key): Retrieve value by key
- getOrDefault(K key, V defaultValue): Retrieve value or return default
- containsKey(K key)
- containsValue(V value)
- remove(K key)
- keySet(): Return all keys
- values(): Return all values
- entrySet(): Return all key-value pairs
- size(): Return number of entries
- isEmpty(): Check if map is empty
- printMap(): Display internal structure (buckets)

The HashMap should handle collisions using separate chaining (linked lists) and support dynamic resizing (rehashing) when the load factor exceeds 0.75.
