/*
This Java program demonstrates a complete Binary Search Tree (BST) implementation:

✅ Features included:

1. **Insertion & Build BST from Array**
   - Builds BST by inserting elements one by one maintaining BST property.

2. **Traversals**
   - Inorder (sorted output for BST)
   - Preorder, Postorder, Level Order (if needed)

3. **Search**
   - Recursively searches for a key in BST.

4. **Deletion**
   - Handles deleting leaf nodes, nodes with one child, and nodes with two children.
   - Uses in-order successor for deletion of nodes with two children.

5. **Root-to-Leaf Paths**
   - Prints all paths from root to each leaf node.

6. **Print Nodes in Range**
   - Efficiently prints nodes within a given range `[k1, k2]` using BST property.

7. **Validation**
   - Checks if a given tree is a valid BST.

8. **Mirror BST**
   - Creates a mirror of the BST by swapping left and right subtrees recursively.

**Example Usage:**
- Build BST from array: `[50, 30, 70, 20, 40, 60, 80, 10, 25, 65]`
- Perform search, deletion, print paths, print range, mirror, and validate BST.

Time Complexity:
- Insertion/Search/Delete: O(H) where H = height of BST
- Traversals: O(N)
- Validation & Mirror: O(N)
