...
You are given the preorder traversal of a binary tree where null nodes are represented by -1.  
Your tasks are:

1. Build the binary tree from the given preorder array.
2. Print all nodes at the Kth level of the tree using two approaches:
   - Approach 1: Standard recursive level print.
   - Approach 2: Optimized recursive approach with current level tracking.
3. Find the Lowest Common Ancestor (LCA) of two nodes using two methods:
   - Method 1: Using paths from root to nodes.
   - Method 2: Using a bottom-up recursive approach.
4. Calculate the minimum distance between two nodes in the binary tree.
5. Find the Kth ancestor of a given node.
6. Transform the binary tree into a Sum Tree, where each node contains the sum of values of its left and right subtrees in the original tree.
7. Perform Preorder traversal to verify tree transformations.

Example Input:
Preorder Array: [1, 2, 4, -1, -1, 5, -1, -1, 3, -1, 6, -1, -1]
Kth Level: 2
Nodes for LCA: (4,5) and (4,6)
Nodes for Distance: (4,6)
Node for Kth Ancestor: 5, k=2

Example Output:
Kth Level (level=3): 4 5 6
Kth Level Optimized (level=2): 2 3
LCA1 of (4,5): 2
LCA2 of (4,6): 1
Distance between 4 and 6: 3
Kth Ancestor of node=5 with k=2: 1
After Transform to Sum Tree, Preorder Traversal: 20 9 0 0 5 0 0 3 0 0
*/
