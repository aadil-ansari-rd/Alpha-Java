...
Key Concepts:
-------------
1. Circular queue allows efficient utilization of space by wrapping around the array.
2. 'front' points to the first element, 'rear' points to the last element.
3. Queue is empty when front == -1.
4. Queue is full when (rear + 1) % size == front.

Operations:
-----------
- enqueue(val): Adds an element at rear (O(1)).
- dequeue(): Removes element from front (O(1)).
- peek(): Returns front element without removing (O(1)).
- display(): Prints elements from front to rear, handling wrap-around.

Special Cases:
--------------
- Single element: After dequeue, reset front and rear to -1.
- Wrap-around: Increment indices modulo size.
- Reuse: After emptying, queue can be reused normally.
