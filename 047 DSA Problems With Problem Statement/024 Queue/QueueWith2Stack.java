...
Description:
Implement a queue using two stacks. You need to implement two approaches:

1. Lazy Enqueue, Eager Dequeue:
   - Enqueue operation: O(1)
   - Dequeue / Peek: Amortized O(1), Worst case O(n)
   - Push new elements into stack s1. 
   - During dequeue/peek, if s2 is empty, transfer all elements from s1 to s2, then pop/peek from s2.

2. Eager Enqueue, Lazy Dequeue:
   - Enqueue operation: O(n)
   - Dequeue / Peek: O(1)
   - During enqueue, move all elements from s1 to s2, push new element into s1, then move elements back from s2 to s1.
   - Dequeue/peek directly from s1.

Example:
Input:
Enqueue(10), Enqueue(20), Enqueue(30), Peek(), Dequeue(), Display()
