...
2. Queue Class:
   - Uses `head` (front) and `tail` (rear) references.
   - isEmpty(): Checks if queue has no elements.
   - isFull(): Always false (dynamic memory).
   - enqueue(value): Adds element at rear.
     * If queue empty, head and tail point to new node.
     * Else, tail.next points to new node and tail updated.
   - dequeue(): Removes element from front.
     * Returns -1 if queue is empty.
     * Updates head. If only one element, resets head and tail.
   - peek(): Returns front element without removing.
   - display(): Prints elements from front to rear.

3. Main Usage:
   - Enqueue elements, peek front, dequeue elements.
   - Handles empty queue cases gracefully.
   - Can reuse after emptying.

Key Points:
   - Dynamic size (linked list).
   - O(1) enqueue and dequeue.
   - Efficient for large queues without fixed array size.
*/
