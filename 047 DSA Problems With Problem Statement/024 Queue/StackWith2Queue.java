...
Description:
Implement a stack data structure using two queues. You must provide two approaches:

1. Costly Push (Efficient Pop):
   - push(x): O(n) 
     Push element into q2, move all elements from q1 to q2, then swap q1 and q2.
   - pop()/peek(): O(1) 
     Directly remove or peek from the front of q1.

2. Costly Pop (Efficient Push):
   - push(x): O(1) 
     Simply enqueue into q1.
   - pop(): O(n) 
     Move all elements except the last one into q2, remove the last, then swap q1 and q2.
   - peek(): O(n) 
     Similar to pop but put the last element back after checking.

Example:
Input:
push(10), push(20), push(30), peek(), pop(), display(), push(40), pop(), pop(), pop(), pop()

Output (Costly Push version):
[30, 20, 10]
Peek: 30
Pop: 30
[20, 10]
