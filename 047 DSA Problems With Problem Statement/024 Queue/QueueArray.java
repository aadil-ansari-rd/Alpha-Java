...
1. Structure:
   - An array `queue[]` holds elements.
   - `front` points to the first element.
   - `rear` points to the last element.
   - `size` defines maximum capacity.

2. Operations:
   - `enqueue(int value)` - Adds element at rear (O(1)).
     * If queue is empty, front is set to 0.
     * If full, prints "Queue is full".
   - `dequeue()` - Removes element from front using front pointer (O(1)).
     * Resets front and rear if queue becomes empty.
   - `dequeue2()` - Removes element by shifting all elements (O(n)).
     * Slower but keeps front at 0 for convenience.
   - `peek()` - Returns front element (O(1)).
   - `display()` - Prints elements from front to rear (O(n)).

3. Notes:
   - `dequeue()` is efficient but front moves forward; resetting needed when empty.
   - `dequeue2()` is simple but slower due to shifting.

4. Usage Example:
   - Enqueue: 10, 20, 30
   - Dequeue (O(1)): removes 10
   - Peek: returns 20
   - Dequeue2 (O(n)): removes 20
   - Fill until full and then empty completely
*/
