...
You are given the preorder traversal of a binary tree in which null nodes are represented by -1.  
Your task is to:

1. Build the binary tree from the given preorder array.
2. Implement two approaches to calculate the diameter of the tree:
   - Approach 1: O(N^2) time complexity.
   - Approach 2: O(N) time complexity.
3. Check whether a given binary tree (subRoot) is a subtree of another binary tree (root).
4. Print the **Top View** of the binary tree (nodes visible when the tree is viewed from the top).

Example Input:
Main Tree Preorder: [1, 2, 4, -1, -1, 5, -1, -1, 3, -1, 6, -1, -1]  
Subtree Preorder: [3, -1, 6, -1, -1]  

Example Output:
Diameter with TC = O(N^2) : 5  
Diameter with TC = O(N)   : 5  
Is Subtree present? : true  
Top View of Tree: 4 2 1 3 6  
*/
