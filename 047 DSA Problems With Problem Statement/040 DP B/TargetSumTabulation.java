/*
Target Sum Problem (LeetCode 494) using Tabulation / Bottom-Up DP

Problem:
Given an array nums[] and a target S, assign + or - signs to each number so that the sum equals target S.
...
sum(P) - sum(N) = target
sum(P) + sum(N) = totalSum → sum(P) = (target + totalSum) / 2
Now count subsets with sum = (target + totalSum)/2

Approach:
1️⃣ Compute totalSum = sum(nums)
2️⃣ If (target + totalSum) % 2 != 0 or |target| > totalSum → return 0
3️⃣ Define subsetSum = (target + totalSum) / 2
4️⃣ Use DP table dp[n+1][subsetSum+1]:
   - dp[i][s] = number of ways to make sum s using first i numbers
   - dp[i][s] = dp[i-1][s] (exclude) + dp[i-1][s-nums[i-1]] (include if nums[i-1] ≤ s)
5️⃣ Answer = dp[n][subsetSum]
...
Output: 3 (ways: [+1,+1,-2,+3], [+1,-1,+2,-3], [-1,+1,+2,+1])
*/
