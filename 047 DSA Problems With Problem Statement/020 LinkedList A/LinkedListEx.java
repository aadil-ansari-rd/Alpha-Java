/*
Problem Name: Singly Linked List Operations
Problem Statement:
-----------------
Implement a singly linked list with multiple operations, including:

1. Adding elements:
   - addStart(data): Add a node at the beginning.
   - addLast(data): Add a node at the end.
   - addAtIndex(data, index): Add a node at a specific index.

2. Removing elements:
   - removeStart(): Remove the first node.
   - removeEnd(): Remove the last node.
   - deleteNthFromEnd(idx): Delete the N-th node from the end.

3. Searching elements:
   - itrSearch(key): Iterative search for a value.
   - recSearch(key): Recursive search for a value.

4. Linked list utilities:
   - getSize(): Get the current size of the list.
   - printList(): Print the list elements.
   - reverse(): Reverse the linked list iteratively.
   - reverse2(): Reverse the linked list recursively.
   - checkPallindrome(): Check if the linked list forms a palindrome.

Example:
LinkedList: 1 -> 2 -> 3 -> 2 -> 1 -> null
checkPallindrome() returns true.

Notes:
- The linked list maintains head and tail references.
- Supports backtracking pointers when reversing or deleting nodes.
- Operations should handle empty list and single-node corner cases.
*/
